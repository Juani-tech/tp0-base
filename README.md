...
```bash
sh generar-compose.sh <output_file> <num_clients>
```
...
- Los demas ejercicios se ejecutan con los make provistos por la catedra

## Protocolo

- Un mensaje del protocolo consta de:

  - [largo] (en bytes): cantidad predefinida en 6 (en ambos configs)
  - [tipo]: Identifica al mensaje para su demultiplexacion por el servidor
  - [data]: El cuerpo de la peticion, contiene los datos necesarios para concretar la respuesta por parte del servidor.

- Estos estan separados en un formato csv (comma separated values), teniendo en el body (siguiendo este formato) en cada campo, un par KEY=VALUE.
- Ademas, cada mensaje esta delimitado por un \n

- Ejemplos de mensajes (sin sus largos):
